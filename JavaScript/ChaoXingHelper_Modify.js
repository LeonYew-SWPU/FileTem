// ==UserScript==
// @name         🥇超星/学习通网课小助手|修复视频播放|自动跳转任务点|章测字体解密|自动答题|题库覆盖率高|逐渐支持更多平台
// @namespace    noshuang
// @version      4.3.1
// @author       noshuang
...
(e => {
    const styleContent = `
        .log[data-v-d41d5897] {
            el-text[data-v-d41d5897] {
                white-space: normal;
            }
        }
        .setting[data-v-4489784f] {
            margin-top: -8px;
            font-size: 14px;
        }
        .setting[data-v-4489784f] .el-form-item[data-v-4489784f] {
            margin-bottom: 0;
        }
        .question_table[data-v-18523ca7] {
            width: 625px;
        }
        .demo-tabs[data-v-7ebed5db] {
            margin-right: 10px;
        }
        .main-page[data-v-9997d2af] {
            z-index: 1000;
            position: fixed;
            display:none; // 隐藏界面
        }
        .main-page .overlay[data-v-9997d2af] {
            position: fixed;
            top: 0;
            left: 0;
            right: 0;
        ...
        .main-page .el-card .card-header[data-v-9997d2af] {
            display: flex;
        ...
        .main-page .el-card .card-header .title[data-v-9997d2af] {
            font-size: 14px;
            display: flex;
            align-items: center;
        ...
        .main-page[data-v-9997d2af] .el-card__header {
            background-color: #1f71e0;
            color: #fff;
            padding: 7px 10px 7px 16px;
            margin: 0;
        }
        .main-page[data-v-9997d2af] .el-card__body {
            padding: 0 6px 20px 16px;
        }
        .main-page[data-v-9997d2af] .el-tabs__nav-wrap:after {
            height: 1px;
        }
        .main-page[data-v-9997d2af] .el-tabs__active-bar {
        ...
        .main-page[data-v-9997d2af] .el-tabs__item.is-top {
            font-weight: 400;
            color: #4e5969;
            padding: 0 8px 0 12px;
        }
        .main-page[data-v-9997d2af] .el-tabs__item.is-active {
            font-weight: 500;
            color: #176ae5;
            padding: 0 8px 0 12px;
        }
        .content[data-v-9997d2af] {
            width: 370px;
        }`;
    const styleElement = document.createElement("style");
    styleElement.dataset.source = "vite-plugin-monkey";
...
(function (vue, pinia, rxjs, md5, ElementPlus) {
    ...
    const useConfigStore = pinia.defineStore("configStore", {
        state: () => {
            ...
            let globalConfig = defaultConfig; //全局配置
            const storedConfig = localStorage.getItem("config"); // 本地存储配置
            if (storedConfig) {
                try {
                    const parsedStoredConfig = JSON.parse(storedConfig); // String -> JSON对象
        ...
        const processIframe = async (iframe) => {
            ...
            if (parentClass.includes("ans-job-finished")) {
                ...
                if (configStore.platformParams.cx.parts[0].params[2].value) {
                    if (iframeSrc.includes("api/work")) {
                        logStore.addLog("只答题，不做其他已开启", "primary");
                    }
                } else {
    ...
    const useCxExamLogic = async () => {
        ...
        logStore.addLog(`正在解析题目, 请等待5s`, "warning");
        if (configStore.platformParams.cx.parts[1].params[1].value) {
            // 如果自动答题开启
            await new CxQuestionHandler("ks").init();   // 创建搜题进程
            logStore.addLog("自动答题已开启", "success");
            ...
            logStore.addLog("已经关闭自动答题", "danger");
        }
    };
    ...
    const CardBody = /* @__PURE__ */ _export_sfc(_sfc_main$3, [["__scopeId", "data-v-7ebed5db"]]);
    ...
    document.onkeydown = function (e) {
        if (e.key == '`') {
            // 读取当前配置
            let currentConfigStr = localStorage.getItem("config"); // 获取当前配置
            let currentConfig = JSON.parse(currentConfigStr); // 转换为对象
            console.log("currenConfig 读取为:" + currentConfig.platformParams.cx.parts[1].params[1].value);

            // 取反并修改currentConfig
            let isOKtoSearch = currentConfig.platformParams.cx.parts[1].params[1].value; // 获取当前是否可以搜索
            isOKtoSearch = !isOKtoSearch; 
            currentConfig.platformParams.cx.parts[1].params[1].value = isOKtoSearch;

            // 保存修改后的currentConfig
            console.log("currenConfig 修改为:" + currentConfig.platformParams.cx.parts[1].params[1].value);
